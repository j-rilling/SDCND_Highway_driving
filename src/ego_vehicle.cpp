...
vector<vector<double>> ego_vehicle::GetNewPosPoints(double x0, double y0, double th0, 
        const vector<double> &previousXpoints, const vector<double> &previousYpoints, 
...
bool ego_vehicle::getVehicleAhead(const vector<vector<double>> &otherVehicles, 
...
vector<double> ego_vehicle::getKinematicsOfLane(const vector<vector<double>> &otherVehicles, unsigned int lane) {
    ...
    if (getVehicleAhead(otherVehicles, lane, vehicle_ahead)) {
...
trajectoryInfo ego_vehicle::keepLaneTraj(double lastPathSize, const vector<vector<double>> &otherVehicles) {
    vector<double> new_vel_acc = getKinematicsOfLane(otherVehicles, this->current_lane);
...
trajectoryInfo ego_vehicle::prepLaneChangeTraj(string state, double lastPathSize, const vector<vector<double>> &otherVehicles) {
    ...
    vector<double> curr_lane_vel_acc = getKinematicsOfLane(otherVehicles, this->current_lane);
    vector<double> vehicle_behind;
    if (getVehicleBehind(otherVehicles, this->current_lane, vehicle_behind)) {
        ...
        vector<double> next_lane_vel_acc = getKinematicsOfLane(otherVehicles, new_lane);
...
trajectoryInfo ego_vehicle::laneChangeTraj(string state, double lastPathSize, const vector<vector<double>> &otherVehicles) {
    ...
    if (getVehicleAhead(otherVehicles, new_lane, vehicle_ahead_new_lane) || 
        getVehicleBehind(otherVehicles, new_lane, vehicle_behind_new_lane)) {
        //std::cout << "Lane occupied. Stay on current lane" << std::endl;
        vector<double> old_lane_vel_acc = getKinematicsOfLane(otherVehicles, this->current_lane);
        ...
        vector<double> new_lane_vel_acc = getKinematicsOfLane(otherVehicles, new_lane);
...
double ego_vehicle::laneChangeWhenSlowCost(trajectoryInfo trajectory, const vector<vector<double>> &otherVehicles) {
    ...
    double diff_target_w_current_speed_h2 = pow(this->target_speed_xy - this->current_speed_xy,2);
    double total_lane_change = (diff_final_w_current_lane + diff_final_w_current_lane);
    double cost = logistic(diff_target_w_current_speed_h2*total_lane_change/(pow(this->target_speed_xy,2)*(this->lanes_quantity-1)*2.0));
    return cost;
}
...
// takes the velocity and acceleration of the new lane, which happens before the ego vehicle is completelly on the 
// new lane.
